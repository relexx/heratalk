...
class RingBufferLoggerTest {
    ...
    @Test
    fun `d emits DEBUG entry when called`() = runTest {
        logger.entries.test {
    ...
    @Test
    fun `w emits WARN entry with throwable when called`() = runTest {
    ...
    @Test
    fun `e emits ERROR entry without throwable when called`() = runTest {
        logger.entries.test {
    ...
    @Test
    fun `multiple log calls emit entries in order`() = runTest {
        logger.entries.test {
            logger.d("T", "first")
            logger.d("T", "second")
            logger.d("T", "third")
    ...
    @Test
    fun `entries beyond MAX_ENTRIES cause oldest to be dropped`() = runTest {
    ...
    @Test
    fun `log entry timestamp is a positive epoch millis value`() = runTest {
